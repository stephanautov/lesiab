// path: nodes/ai.langgraph.flow.ts
/**
 * NODE: ai.langgraph.flow
 * Phase: codeGeneration
 *
 * Purpose:
 *  - Implement a LangGraph-style codegen pipeline with four stages:
 *      (a) analyzeRequest → (b) proposeFiles → (c) synthesizeContent → (d) verifyTypes
 *  - Expose a factory `buildCodegenFlow({ writeFile, formatRepo })` so the app
 *    can plug in its own file writer/formatter (ToolNodes).
 *  - Keep deterministic outputs: no random IDs or timestamps.
 *
 * Outputs:
 *  - artifacts/${orc}/repo/lib/ai/flows/codegenFlow.ts
 *
 * Notes:
 *  - We **dynamically import** `@langchain/langgraph` at runtime. If the package
 *    isn’t installed yet, we fall back to a simple in-file pipeline runner so
 *    this module still compiles and can be exercised in tests.
 */
...
/** ---------- Code that will be written into the repo (the app consumes this) ---------- */
const CODEGEN_FLOW_TS = lf(`// path: lib/ai/flows/codegenFlow.ts
/**
...
export const AiLanggraphFlowNode: NodeSpec<unknown, { files: string[] }> = {
  id: "ai.langgraph.flow",
  phase: "codeGeneration",
  estimate: () => ({ tokens: 950, usd: 0.004 }),
...
export default AiLanggraphFlowNode;
