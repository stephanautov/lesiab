...
export type OrchestrationStatus = 'pending' | 'running' | 'complete' | 'failed' | 'cancelled'
...
type SerializableCheckpoint = Omit<Checkpoint, 'completedNodes' | 'updatedAt'> & {
...
const BlueprintSchema = z.object({
  ...
  nodes: z.array(z.object({
    id: NodeIdSchema,
    phase: z.enum(['processResponses', 'analyze', 'validate', 'plan', 'execute', 'codeGeneration', 'integrate', 'finalize'])
  })),
...
export function validateBlueprint(bp: Blueprint): void {
  ...
  for (const node of bp.nodes) {
    if (nodeIds.has(node.id)) {
      throw new ValidationError(`Duplicate node ID: ${node.id}`, new z.ZodError([]))
    }
    nodeIds.add(node.id)
  }
  ...
  for (const edge of bp.edges) {
    ...
    if (edge.from === edge.to) {
      throw new ValidationError(`Self-loop detected on node ${edge.from}`, new z.ZodError([]))
    }
    
    // Check that edge references valid nodes
    if (!nodeIds.has(edge.from)) {
      throw new ValidationError(`Edge references unknown source node: ${edge.from}`, new z.ZodError([]))
    }
    if (!nodeIds.has(edge.to)) {
      throw new ValidationError(`Edge references unknown target node: ${edge.to}`, new z.ZodError([]))
    }
  }
}
...
export class DependencyResolver {
   ...
   */
  detectCycles(nodes: NodeId[], edges: Array<{ from: NodeId; to: NodeId }>): string[][] {
...
export class TaskPool {
  ...
  async acquire(n: number = 1): Promise<() => void> {
    if (n > this.maxConcurrency) {
      throw new Error(`Cannot acquire ${n} permits from pool of ${this.maxConcurrency}`)
    }
...
export function calculateBackoff(attempt: number, base: number = 250, max: number = 5000): number {
...
export async function retry<T>(
  ...
  for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
    try {
      ...
      const baseDelay = config.backoffMs[Math.min(attempt - 1, config.backoffMs.length - 1)]
      const delay = config.jitter ? calculateBackoff(attempt, baseDelay) : baseDelay
...
export class SimpleMetricsCollector implements MetricsCollector {
  ...
  getMetrics(): Record<string, { count: number; sum: number; avg: number; p50?: number; p95?: number }> {
...
export class BudgetTracker {
  ...
  async ensureWithinBudget(): Promise<void> {
    ...
    if (this.usedTokens > this.config.maxTokens) {
      throw new BudgetExceededError('tokens', this.usedTokens, this.config.maxTokens, this.orchestrationId)
    }
    
    // Check USD budget
    if (this.usedUsd > this.config.maxUsd) {
      throw new BudgetExceededError('usd', this.usedUsd, this.config.maxUsd, this.orchestrationId)
    }
    
    // Check time budget
    const elapsed = Date.now() - this.startTime
    if (elapsed > this.config.maxWallClockMs) {
      throw new BudgetExceededError('time', elapsed, this.config.maxWallClockMs, this.orchestrationId)
    }
  }
...
export function computePlanHash(blueprint: Blueprint, batches: NodeId[][]): string {
...
export interface Repository {
  ...
  listArtifacts(orchestrationId: OrchestrationId): Promise<Array<{path: string; size: number}>>
...
export class SupabaseRepository implements Repository {
  ...
  async listArtifacts(orchestrationId: OrchestrationId): Promise<Array<{path: string; size: number}>> {
    const { data, error } = await this.supabase
      .from('artifacts')
      .select('path, bytes')
  ...
  async signArtifact(artifact: {path: string}, ttl: number = 900): Promise<string> {
    const { data, error } = await this.supabase.storage
      .from('artifacts')
...
export class OrchestrationEngine {
  ...
  constructor(
    ...
    if (!budgetValidation.success) {
      throw new ValidationError('Invalid engine budget configuration', budgetValidation.error)
    }
    ...
    if (config.maxConcurrencyPerOrchestration < 1 || config.maxConcurrencyPerOrchestration > config.maxConcurrencyGlobal) {
      throw new Error('maxConcurrencyPerOrchestration must be between 1 and maxConcurrencyGlobal')
    }
  ...
  async execute(args: {
    ...
    try {
      ...
      logger.info(`Executing ${batches.length} batches with ${args.blueprint.nodes.length} nodes, plan: ${planHash}`)
      ...
      if (existingCheckpoint) {
        ...
        logger.info(`Resuming from batch ${startBatchIndex} with ${completedNodes.size} completed nodes`)
      } else {
      ...
      for (let batchIndex = startBatchIndex; batchIndex < batches.length; batchIndex++) {
        ...
        const remainingNodes = batch.filter(nodeId => !completedNodes.has(nodeId))
        
        if (remainingNodes.length === 0) {
          logger.info(`Batch ${batchIndex + 1}/${batches.length} already completed, skipping`)
          continue
        }
        
        logger.info(`Executing batch ${batchIndex + 1}/${batches.length} with ${remainingNodes.length} remaining nodes`)

        // Execute nodes in parallel with per-orchestration concurrency limit
        const orchestrationPool = new TaskPool(this.config.maxConcurrencyPerOrchestration)
        const nodePromises = remainingNodes.map(async (nodeId) => {
          ...
          try {
            ...
            // Record pool metrics
            this.metrics.recordGauge('pool_global_inuse', this.globalPool.stats.inUse)
            this.metrics.recordGauge('pool_local_inuse', orchestrationPool.stats.inUse)
          }
        })
        ...
        if (failed.length > 0) {
          const error = failed[0].status === 'rejected' ? failed[0].reason : new Error('Batch execution failed')
          logger.error(`Batch ${batchIndex} had ${failed.length} failures`, error)
          throw error
        }
        ...
        if (batchIndex < batches.length - 1) {
          await context.checkpoint.save({
            phase: this.getPhaseForBatch(args.blueprint, batches[batchIndex + 1]),
      ...
      // Mark as complete
      await this.repository.updateStatus(args.orchestrationId, 'complete', 'All batches executed successfully')
      ...
      const artifacts = await this.repository.listArtifacts(args.orchestrationId)
      ...
      logger.info(`Orchestration completed successfully with ${signedUrls.length} artifacts`, {
        executionTime,
        planHash,
        phases: phaseCount
      })
      ...
      await this.repository.updateStatus(args.orchestrationId, 'failed', statusReason)
    ...
    try {
      ...
      const output = await retry(
        async () => {
          ...
          nodeLogger.info(`Executing node ${nodeId} (attempt ${attemptNo}/${retryConfig.maxAttempts})`, {
            timeout,
            phase: node.phase
          })
      ...
      nodeTimer()
      nodeLogger.error(`Node ${nodeId} failed after ${attemptNo} attempts`, error as Error)
  ...
  getStats() {
    return {
      ...
      config: {
        ...
        maxConcurrencyPerOrchestration: this.config.maxConcurrencyPerOrchestration,
